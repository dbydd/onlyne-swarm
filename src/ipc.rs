...
fn reap_previous_run(sched: &Arc<Sched>) {
    ...
    for r in rows {
            ...
            && crate::orca_term::is_alive(&r.terminal)
        {
            ...
            sched.hop_since.lock().unwrap().insert(r.task_id.clone(), ("busy".into(), std::time::Instant::now()));
            sched.emit(
                "hop_state",
            ...
            sched.note_alert(format!(
                "adopted: {} {}", &r.task_id[..8.min(r.task_id.len())], r.to_ws
            ));
            continue;
        }
...
fn hop_timeouts(sched: &Arc<Sched>) {
    ...
    for (task_id, hop, since) in clocks {
        ...
        match hop.as_str() {
            "ready" if elapsed >= Duration::from_secs(30) => {
                sched.emit("task_stalled_ready_timeout", serde_json::json!({"task_id": task_id}));
            ...
            "busy" => {
                let role = sched.db.get(&task_id).ok().flatten().map(|r| r.to_ws).unwrap_or_default();
                let Some(limit) = sched.busy_limit_secs(&role) else { continue };
                ...
                if due {
                    sched.overlong_last.lock().unwrap().insert(task_id.clone(), now);
                    sched.emit("hop_overlong", serde_json::json!({
    ...
    sched.overlong_last.lock().unwrap().retain(|k, _| overlong_seen.contains(k));
}
    ...
    loop {
        ...
        match op {
            ...
            "status" => {
                ...
                let overlong: std::collections::HashSet<String> =
                    sched.overlong_last.lock().unwrap().keys().cloned().collect();
...
mod tests {
    ...
    fn hop_timeouts_fail_ready_stall_and_respect_pending_idle() {
        ...
        s.hop_since.lock().unwrap().insert(
            "rdy".into(),
            ("ready".into(), std::time::Instant::now() - std::time::Duration::from_secs(31)),
        );
        ...
        s.hop_since.lock().unwrap().insert(
            "idl".into(),
            ("idle".into(), std::time::Instant::now() - std::time::Duration::from_secs(120)),
        );
        s.idle_pending_exit.lock().unwrap().insert("idl".into());
        hop_timeouts(&s);
        assert_eq!(s.db.get("rdy").unwrap().unwrap().state, TaskState::Failed, "ready stall fails");
        assert!(!s.terminals.lock().unwrap().contains_key("rdy"), "CR1: terminal recycled");
        assert_eq!(s.db.get("idl").unwrap().unwrap().state, TaskState::Running, "CR3: pending idle untouched");
    }
