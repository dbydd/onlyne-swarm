...
│ └─ reviewer ● ╴╴╴╴╴ │ …                                       │
│    ╰╴ planner（回边）│                                         │
├─ daemons/terminals ─┴─ ledger tail ───────────────────────────┤
│ planner: daemon ready, terminal abc123 running                │
│ worker: daemon starting…                                      │
│ ledger: done ccccdddd… / failed … / cancelled …               │
└─ [c]ancel [t]oggle-swarm [q]uit ──────────────────────────────┘
```

- 左树：workspace 层级；`●` daemon 在线，`○` 离线；`back_edges` 用虚线 + `╰╴` 标注。
- 右上任务表：`task_id` 短显（前 8）、`from → to`、`attempt`、`state`
 （pending/running/done/failed/cancelled/closed，closed 默认折叠）、`xfer` 血缘短 id。
- 右下：daemon/terminal 存活、`swarm_ready` 超时计数、ledger tail（done/failed/cancelled）。
- 状态栏：`c` 取消选中任务血缘族，`t` 切换选中 workspace 的 swarm 开关，`q` 退出。

## 2. 数据源

2s 间隔全量 `list_workspaces` + `list_tasks` 轮询刷新（`subscribe` 流为后续增量源，当前版本轮询已满足排障需求）。按键 `↑↓/jk` 选任务，`c` 取消选中任务血缘族，`t` 切换 root swarm 开关，`q` 退出。
